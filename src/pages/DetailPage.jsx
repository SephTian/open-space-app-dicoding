...
function DetailPage() {
  const { id } = useParams();
  const { talkDetail = null, authUser } = useSelector((state) => state); // get talkDetail and authUser state from store
  const dispatch = useDispatch(); // get dispatch function from store

  useEffect(() => {
    // dispatch async action to get talk detail by id
    dispatch(asyncReceiveTalkDetail(id));
  }, [id, dispatch]);

  const onLikeTalk = () => {
    // dispatch async action to toggle like talk detail
    dispatch(asyncToogleLikeTalkDetail());
  };

  const onReplyTalk = (text) => {
    // dispatch async action to add reply talk
