...
function HomePage() {
  const { talks = [], users = [], authUser } = useSelector((state) => state); // get talks, users, and authUser state from store

  const dispatch = useDispatch(); // get dispatch function from store

  useEffect(() => {
    // dispatch async action to populate talks and users data
    dispatch(asyncPopulateUsersAndTalks());
  }, [dispatch]);

  const onAddTalk = (text) => {
    // dispatch async action to add talk
    dispatch(asyncAddTalk({ text }));
  };

  const onLike = (id) => {
    // dispatch async action to toggle like talk
